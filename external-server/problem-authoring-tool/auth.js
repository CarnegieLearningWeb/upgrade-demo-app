...
export function createAppAuth({
  ...
  label = 'app',
  allowedEmailDomain = 'carnegielearning.com',
} = {}) {
  ...
  const loginHandler = async (req, res) => {
    ...
    if (allowedEmailDomain && !payload.email?.toLowerCase().endsWith(`@${allowedEmailDomain}`)) {
      return res.status(403).json({
        error: {
          code: 'email_domain_not_allowed',
          message: `Please sign in with a ${allowedEmailDomain} Google account`,
        },
      })
    }
